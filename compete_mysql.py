...
class BS_Compete(currency.BS_currency):#生意参谋_竞争
    ...
    def cruxIndex(self,i):#关键指标
        ...
        for j in l02:
            if j=='店铺日期':
                ...
                l04+=[b_data[j][1]]
        sql="insert into cruxIndex values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
    ...
    def commodityList(self,i):#TOP商品榜
        ...
        for j in range(2):
            ...
            time.sleep(2)
            # 店铺top前十
            sql="insert into commodityList values(%s,%s,%s,%s)"
    ...
    def transactionComposition(self,i):#交易构成
        ...
        bd=self.driver.find_element_by_css_selector('span.ebase-Selector__title').text#本店名称
        sql="insert into transactionComposition values(%s,%s,%s)"
        ...
        sql="insert into price_band values(%s,%s,%s)"
        ...
        for j in range(len(li)):
            ...
            for k in tbody:
                ...
                if '流量' in xm:
                    ...
                    Li[0][k[0]]['支付金额']=k[3]
                    Li[1][k[0]]['支付金额']=''
        sql="insert into sourceOfEntry values(%s,%s,%s,%s,%s,%s,%s,%s,%s)"
